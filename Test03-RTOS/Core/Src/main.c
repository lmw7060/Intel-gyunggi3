...
void StartTask02(void const * argument)
{
  ...
  for(;;)
  {
	  if(osSemaphoreWait(myBinarySem01Handle, 0) == osOK)
	  {
		  ...
		  //HAL_GPIO_TogglePin(LDg_GPIO_Port, LDg_Pin); HAL_Delay(500);
		  HAL_GPIO_WritePin(LDg_GPIO_Port, LDg_Pin, 1); HAL_Delay(100);
		  HAL_GPIO_WritePin(LDg_GPIO_Port, LDg_Pin, 0); HAL_Delay(100);
		  osSemaphoreRelease(myBinarySem01Handle);
	  }
	  osDelay(1);
  }
...
void StartTask03(void const * argument)
{
  ...
  for(;;)
  {
	  if(osSemaphoreWait(myBinarySem01Handle, 0) == osOK)
	  {
		  ...
		  //HAL_GPIO_TogglePin(LDy_GPIO_Port, LDy_Pin); HAL_Delay(100);
		  HAL_GPIO_WritePin(LDy_GPIO_Port, LDy_Pin, 1); HAL_Delay(100);
		  HAL_GPIO_WritePin(LDy_GPIO_Port, LDy_Pin, 0); HAL_Delay(100);
		  osSemaphoreRelease(myBinarySem01Handle);
	  }
	  osDelay(1);
  }
